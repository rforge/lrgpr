...
void parse_TPED( Rcpp::NumericMatrix *X, const long line_number, const string &line, const char *delimiters, long *n_tokens, SNPinfo *markerInfo, bool *foundMissingData, const vector<string> &markerNames){
...
void parse_GEN( Rcpp::NumericMatrix *X, const long line_number, const string &line, const char *delimiters, long *n_tokens, SNPinfo *markerInfo, bool *foundMissingData, const vector<string> &markerNames){
...
void parse_DOSAGE( Rcpp::NumericMatrix *X, const long line_number, const string &line, const char *delimiters, long *n_tokens, SNPinfo *markerInfo, bool *foundMissingData, const vector<string> &markerNames){
...
RcppExport SEXP R_convertToBinary( SEXP fileName_, SEXP fileNameOut_, SEXP format_, SEXP isZipFile_){
	...
	do{
		...
		if( format == "DOSAGE" && line.compare(0,9,"SNP A1 A2") == 0){
			n_lines--;
		}

		if( n_lines % 10000 == 0 )
			cout <<"\rReading line " << n_lines << "             " << flush;
		
	}while( getline(ifstrm, line) );

	cout <<"\rReading line " << n_lines << endl;

	long n_indivs; 

	if( format == "TPED" ) 			n_indivs = ( n_tokens - 4)/2; // TPED 
	else if( format == "GEN" )		n_indivs = ( n_tokens - 5)/3; // GEN  / OXFORD
	else if( format == "DOSAGE" )	n_indivs = ( n_tokens - 4)/2; // dosage,gz  / MACH
	...
	for(long count=0; count<n_lines; count++){

		if( ! getline(ifstrm2, line) ) break;
		
		if( format == "TPED" ){
			
			parse_TPED( &X, 0, line, " ", &n_tokens, markerInfo, &foundMissingData, markerNames);
			...
			parse_GEN ( &X, 0, line, " ", &n_tokens, markerInfo, &foundMissingData, markerNames);
		
		}else if( format == "DOSAGE" ){

			parse_DOSAGE ( &X, 0, line, " ", &n_tokens, markerInfo, &foundMissingData, markerNames);
		}
	...
	print_progress( n_lines, n_lines, 25, start_time);
	cout << endl;
